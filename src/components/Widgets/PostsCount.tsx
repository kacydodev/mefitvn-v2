...
const PostsCount = async () => {
  ...
  const recentlyPublished = posts.docs.find((post: Post) => post._status === 'published')
  ...
  return (
    <div className='card posts-count-container'>
      <div className='post-count-content'>
        ...
        <Link href={`/admin/collections/posts/${recentlyPublished?.id}`}>{recentlyPublished && truncateByWords(recentlyPublished.title, 4)}</Link>
        <p>Latest draft:</p>
        <Link href={`/admin/collections/posts/${latestDraft?.id}`}>{latestDraft && formatRelative(latestDraft.updatedAt, new Date())}</Link>
      </div>
